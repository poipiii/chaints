...
class DeliveryForm(Form):
    ...
    city= StringField('City',[validators.length(min=1,max=20),validators.DataRequired()])
    state= StringField('State',[validators.length(min=1,max=10),validators.DataRequired()])
    zip= StringField('Zip',[validators.length(min=1,max=10),validators.DataRequired()])
...
class Payment_Form(Form):
    ...
    expiry=IntegerField('Date Of Expiry',validators=[validators.InputRequired()])
